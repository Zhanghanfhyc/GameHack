...
//    static float C = 5;   // this is the constant that regulates exploration vs exploitation, it must be tuned for each domain
    ...
    public DownsamplingUCTNode(int currPlayer, int ourID, GameState a_gs, DownsamplingUCTNode a_parent, long MAXACTIONS, float bound) throws Exception {
        parent = a_parent;
        gs = a_gs;
    ...
    public DownsamplingUCTNode UCTSelectLeaf(int currPlayer, int ourID, long MAXACTIONS, long cutOffTime, int max_depth) throws Exception {
        ...
        if (moveGenerator!=null && actions==null) {
            ...
            if (moveGenerator.getSize()>2*MAXACTIONS) {
                for(int i = 0;i<MAXACTIONS;i++) {
                    actions.add(moveGenerator.generateNextAction()); //TODO needs to select random move, not next one
                }
            } else {      
                PlayerAction pa = null;
                long count = 0;
                do{
                    pa = moveGenerator.generateNextAction();
                    if (pa!=null) {
                        actions.add(pa);
                        count++;
        ...
        if (hasMoreActions) {
            ...
            if (children.size()>=actions.size()) {
                ...
                DownsamplingUCTNode node = new DownsamplingUCTNode(currPlayer, ourID, new GameState(gs2), this, MAXACTIONS, evaluation_bound);
                children.add(node);
                return node;                
            }            
        }
        ...
        return best.UCTSelectLeaf(currPlayer, ourID, MAXACTIONS, cutOffTime, max_depth);
    }    
